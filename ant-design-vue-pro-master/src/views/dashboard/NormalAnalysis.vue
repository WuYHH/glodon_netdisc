  ...
  <div>
    <a-row :gutter="24">
      <a-col :sm="24" :md="12" :xl="6" :style="{ marginBottom: '24px' }">
        <chart-card class="total_card" :loading="loading" title="文件总数" :total="fileCount | NumberFormat">
        </chart-card>
      </a-col>
      <a-col :sm="24" :md="12" :xl="6" :style="{ marginBottom: '24px' }">
        <chart-card class="total_card" :loading="loading" title="占用总空间" :total="fileSizeCount">
        </chart-card>
      </a-col>
      <a-col :sm="24" :md="12" :xl="6" :style="{ marginBottom: '24px' }">
        <chart-card class="total_card" :loading="loading" title="文件类型数" :total="fileTypeCount | NumberFormat">
        </chart-card>
      </a-col>
      <!-- <a-col :sm="24" :md="12" :xl="6" :style="{ marginBottom: '24px' }">
    ...
    <a-card :loading="loading" :bordered="false" :body-style="{padding: '0'}">
      <div class="salesCard">
        <a-tabs default-active-key="1" size="large" :tab-bar-style="{marginBottom: '24px', paddingLeft: '16px'}">
          ...
          <a-tab-pane loading="true" tab="文件类型统计" key="1">
            <a-row>
          ...
          <!-- <a-tab-pane :tab="$t('dashboard.analysis.visits')" key="2">
            <a-row>
              <a-col :xl="16" :lg="12" :md="12" :sm="24" :xs="24">
                <bar :data="fileTypeData2" :title="$t('dashboard.analysis.visits-trend')" />
              </a-col>
              <a-col :xl="8" :lg="12" :md="12" :sm="24" :xs="24">
...
export default {
  ...
  methods: {
    ...
    getFileCount () {
      dashApi.getFileCount(this.user.id).then(res => {
        this.fileCount = res.data
        console.log('文件总数', res)
      })
    },
    getFileCountSize () {
      dashApi.getFileCountSize(this.user.id).then(res => {
        this.fileSizeCount = res.data
        console.log('文件大小总数', res)
      })
    },
    getFileTypeCount () {
      dashApi.getFileTypeCount(this.user.id).then(res => {
        this.fileTypeCount = res.data
        console.log('文件类型', res)
      })
    },
    getTypeCount () {
      dashApi.getTypeCount(this.user.id).then(res => {
        var data = []
