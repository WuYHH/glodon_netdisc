  ...
  <div>
    <a-row :gutter="24">
      <a-col :sm="24" :md="12" :xl="6" :style="{ marginBottom: '24px' }">
        <chart-card :loading="loading" title="文件总数" :total="fileTotal | NumberFormat">
        </chart-card>
      </a-col>
      <a-col :sm="24" :md="12" :xl="6" :style="{ marginBottom: '24px' }">
        <chart-card :loading="loading" title="用户总数" :total="userTotal | NumberFormat">
        </chart-card>
      </a-col>
      <a-col :sm="24" :md="12" :xl="6" :style="{ marginBottom: '24px' }">
        <chart-card :loading="loading" title="占用总容量" :total="sizeTotal">
        </chart-card>
      </a-col>
      <a-col :sm="24" :md="12" :xl="6" :style="{ marginBottom: '24px' }">
        <chart-card :loading="loading" title="类型总数" :total="typeTotal | NumberFormat">
        </chart-card>
      </a-col>
      <!-- <a-col :sm="24" :md="12" :xl="6" :style="{ marginBottom: '24px' }">
    ...
    <a-card :loading="loading" :bordered="false" :body-style="{padding: '0'}">
      <div class="salesCard">
        <a-tabs default-active-key="1" size="large" :tab-bar-style="{marginBottom: '24px', paddingLeft: '16px'}">
          ...
          <a-tab-pane loading="true" tab="用户文件" key="1">
            <a-row>
              <a-col :xl="16" :lg="12" :md="12" :sm="24" :xs="24">
                <bar :data="userFileData" title="用户文件统计" />
              </a-col>
          ...
          <a-tab-pane tab="文件类型" key="2">
            <a-row>
              <a-col :xl="16" :lg="12" :md="12" :sm="24" :xs="24">
                <bar :data="fileTypeData" title="文件类型统计" />
              </a-col>
            </a-row>
          </a-tab-pane>
        </a-tabs>
...
export default {
  ...
  methods: {
    ...
    getFileCount () {
      dashApi.getAdminFileCount().then(res => {
        this.fileTotal = res.data
      })
    },
    getFileCountSize () {
      dashApi.getAdminFileCountSize().then(res => {
        this.sizeTotal = res.data
      })
    },
    getFileTypeCount () {
      dashApi.getAdminFileTypeCount().then(res => {
        this.typeTotal = res.data
      })
    },
    getTypeCount () {
      dashApi.getAdminTypeCount().then(res => {
        var data = []
        for (let i = 0; i < res.data.length; i += 1) {
        data.push({
          x: res.data[i].typeName,
          y: res.data[i].count
        })
      }
      this.fileTypeData = data
    ...
    getUserCount () {
      dashApi.getUserCount().then(res => {
        this.userTotal = res.data
      })
    },
    getUserFileCount () {
      dashApi.getUserFileCount().then(res => {
        for (let i = 0; i < res.data.length; i += 1) {
        this.userFileData.push({
          x: res.data[i].username,
          y: res.data[i].count
        })
      }
      })
